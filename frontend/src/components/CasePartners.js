...
export default function CasePartners({ shipmentId, archived }) {
  const [data, setData] = useState([]); const [companies, setCompanies] = useState([]);
  ...
  const [company, setCompany] = useState({ name: '', type: 'FORWARDER' }); const [companyId, setCompanyId] = useState('');
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const [p,c] = await Promise.all([api.partners(shipmentId), api.companies()]); setData(p); setCompanies(c); }
  ...
  const remaining = companies.filter(c => !data.some(p => p.companyId === c.companyId));
  return <section className="panel case-partners"><div className="panel-header"><div><h2>외부 업체와 담당자</h2><p className="muted">이 건에서 업체별 초대 대기·참여 중 담당자 합계 최대 3명입니다.</p></div><button className="button secondary" disabled={busy || loading} onClick={() => { setError(''); setDecision(null); load(); }}>참여자 새로고침</button></div>
    <div className="panel-body"><ErrorMessage message={error}/>{notice && <p role="status">{notice}</p>}
      {loading ? <LoadingState/> : <>{data.map(p => <article className="partner-card" key={p.casePartnerId}><h3>{p.name} · {typeLabel(p.type)}</h3><p>{p.occupied} / 3명</p>
      ...
      {!archived && !loading && !decision && <>
        ...
        {remaining.length > 0 && <form className="form-stack partner-form" onSubmit={e => { e.preventDefault(); run(() => api.attachCompany(shipmentId, companyId), '이 건에 업체를 연결했습니다.'); }}><h3>등록된 업체 연결</h3><label>외부 업체<select required value={companyId} onChange={e => setCompanyId(e.target.value)}><option value="">업체 선택</option>{remaining.map(c => <option value={c.companyId} key={c.companyId}>{c.name} · {typeLabel(c.type)}</option>)}</select></label><button className="button secondary" disabled={busy || !remaining.some(c => c.companyId === companyId)}>이 건에 업체 연결</button></form>}
        <form className="form-stack partner-form" onSubmit={e => { e.preventDefault(); run(async () => { const c = await api.createCompany(company); setCompany({ name: '', type: 'FORWARDER' }); setCompanyId(c.companyId); }, '업체를 등록했습니다. 아래 선택 목록에서 이 건에 연결해 주세요.'); }}><h3>새 외부 업체 등록</h3><label>업체명<input required maxLength={200} value={company.name} onChange={e => setCompany({ ...company, name: e.target.value })}/></label><label>업체 유형<select value={company.type} onChange={e => setCompany({ ...company, type: e.target.value })}><option value="FORWARDER">포워더</option><option value="CUSTOMS_BROKER">관세사</option></select></label><button className="button secondary" disabled={busy || !company.name.trim()}>업체 등록</button></form>
      </>}
    </div>
  </section>;
}
