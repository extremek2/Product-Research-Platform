...
BEGIN READ ONLY;
SELECT version,success FROM flyway_schema_history WHERE version IN ('14','15','16','17') ORDER BY installed_rank;
...
SELECT count(*) AS legacy_unmapped_contacts FROM external_contact WHERE partner_company_id IS NULL;
SELECT count(*) AS legacy_unmapped_participants FROM case_participant WHERE case_partner_id IS NULL;
...
DO $$ BEGIN
  IF (SELECT count(*) FROM flyway_schema_history WHERE version IN ('14','15','16','17') AND success)<>4 THEN
    RAISE EXCEPTION 'Expected identity migrations V14-V17';
  END IF;
END $$;
COMMIT;
